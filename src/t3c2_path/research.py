...
def _canonical_float(value: float) -> float:
    """Return the portable serialization precision for generated values.

    Libm-backed transforms and long reductions can differ in their final bits
    across operating systems. Ten decimal places preserve more precision than
    this model claims while making the research manifest cross-platform.
    """

    return round(value, CANONICAL_DECIMAL_PLACES)
...
def generate_known_truth_cohort(
    *, n: int = 1_200, seed: int = 20260719
    ...
    for index in range(n):
        baseline = _canonical_float(rng.uniform(35.0, 75.0))
        motivation = _canonical_float(rng.random())
        resources = _canonical_float(rng.random())
        logit = -1.3 + 0.045 * (baseline - 50.0) + 1.5 * motivation + 0.9 * resources
        propensity = _canonical_float(min(0.92, max(0.08, _logistic(logit))))
        assigned = rng.random() < propensity
        received = assigned if rng.random() >= 0.10 else not assigned
        true_effect = _canonical_float(
            2.0 + 1.5 * (1.0 - resources) + 0.5 * motivation
        )
        expected_y0 = _canonical_float(
            18.0 + 0.65 * baseline + 8.0 * motivation + 4.0 * resources
        )
        expected_y1 = _canonical_float(expected_y0 + true_effect)
        shared_noise = _canonical_float(rng.gauss(0.0, 4.0))
        y0 = _canonical_float(expected_y0 + shared_noise)
        y1 = _canonical_float(expected_y1 + shared_noise)
...
def run_known_truth_validation(
    *, n: int = 1_200, seed: int = 20260719
    ...
    return ValidationReport(
        ...
        true_ate=_canonical_float(true_ate),
        naive_difference=_canonical_float(naive),
        aipw_estimate=_canonical_float(aipw.report.estimate),
        aipw_interval_low=_canonical_float(aipw.report.interval_low),
        aipw_interval_high=_canonical_float(aipw.report.interval_high),
        naive_absolute_bias=_canonical_float(abs(naive - true_ate)),
        aipw_absolute_bias=_canonical_float(abs(aipw.report.estimate - true_ate)),
        propensity_min=_canonical_float(min(row.propensity for row in analyzed)),
        propensity_max=_canonical_float(max(row.propensity for row in analyzed)),
